...
```{r vzzzz , echo=FALSE, fig.width=12, fig.height=6, warning=FALSE, message=FALSE}
# What are the genes that determine PC1 and PC2?
VizDimLoadings(seurat_filtered, dims = 1:2)
```

```{r an, echo=FALSE, include=FALSE}
...
```{r ann3, echo=FALSE,fig.width=12, fig.height=6, warning=FALSE, message=FALSE}
#UMAP for PBMC
...
```{r cluster, echo=FALSE,include=FALSE, warning=FALSE}
...
```{r 5marker, echo=FALSE, include=FALSE}
library(dplyr)

# View top 5 markers per cluster
top_markers <- tnbc.markers %>%
  group_by(cluster) %>%
...
```{r  an76, echo=FALSE, include=FALSE}
# Extract the normalized data (after NormalizeData())
expr_matrix <- GetAssayData(seurat_filtered, slot = "data")
...
singleR_labels <- SingleR(test = expr_matrix, ref = ref, labels = ref$label.main)

# Add SingleR labels to metadata
seurat_filtered$SingleR_labels <- singleR_labels$labels



```

```{r anotw, echo=FALSE, include=FALSE}
dbs <- listEnrichrDbs()
...
```{r an7634, echo=FALSE, include=FALSE}
...
```{r annot34 , include=FALSE, warning=FALSE, echo=FALSE}
...
```{r annpot3, echo=FALSE, include=FALSE,fig.width=20, fig.height=10}
...
```{r aannt1 ,echo=FALSE, fig.width=12, fig.height=6, warning=FALSE,message=FALSE}

DimPlot(seurat_filtered, group.by = "SingleR_labels", label = TRUE) +
  ggtitle("UMAP Colored by Auto-Annotated Cell Types (SingleR)") +
  theme_minimal()

```

```{r annt,echo=FALSE,fig.width=12, fig.height=6}
## Part 2b: Finding differences using genes ##
DimPlot(seurat_filtered) + DimPlot(seurat_filtered, group.by = "SingleR_labels")
```
